...
namespace Gwen.Control
{
    ...
    public class ImagePanel : Base
    {
        ...
        {
            if (x < 0 || y < 0 || w < 0 || h < 0 || x + w > m_Texture.Width || y + h > m_Texture.Height) return;
            m_uv[0] = (float)x / (float)m_Texture.Width;
            m_uv[1] = (float)y / (float)m_Texture.Height;
            m_uv[2] = (float)(x + w) / (float)m_Texture.Width;
            m_uv[3] = (float)(y + h) / (float)m_Texture.Height;
        }
        ...
        protected override void Render(Skin.Base skin)
        {
            ...
            skin.Renderer.DrawTexturedRect(m_Texture, RenderBounds,Color.White, m_uv[0], m_uv[1], m_uv[2], m_uv[3]);
        }
